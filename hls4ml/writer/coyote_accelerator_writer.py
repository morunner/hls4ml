    ...
    def write_host_code(self, model):
        ...
        for line in f.readlines():
            ...
            if '// hls-fpga-machine-learning insert I/O size' in line:
                newline = ''
                newline += indent + f'unsigned int in_size = {{ {" ,".join([inp.size_cpp() for inp in model_inputs])} }};\n'
                newline += (
                    indent + f'unsigned int out_sizes[] = {{ {" ,".join([out.size_cpp() for out in model_outputs])} }};\n'
                )
        ...
        for line in f.readlines():
            ...
            if 'myproject' in line:
                ...
                for i, inp in enumerate(model_inputs):
                    ...
                    newline += (
                        indent + f'nnet::copy_data<float, {inp.type.name}, {offset}, {inp.size_cpp()}>(in, {inp.name});\n'
                    )
                ...
                newline = line
                for out in model_outputs:
                    newline += indent + f'nnet::print_result<{out.type.name}, {out.size_cpp()}>({out.name}, fout);\n'
                ...
                for out in model_outputs:
                    newline += (
                        indent + f'nnet::print_result<{out.type.name}, {out.size_cpp()}>({out.name}, std::cout, true);\n'
                    )
