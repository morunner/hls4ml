...
void conv_1d_latency_cl(data_T data[CONFIG_T::in_width * CONFIG_T::n_chan],
    ...
    constexpr int PARALLEL_FACTOR = CONFIG_T::n_partitions / TIME_STEPS; 

    #pragma HLS ARRAY_PARTITION variable=data type=cyclic factor=PARALLEL_FACTOR
    #pragma HLS ARRAY_PARTITION variable=res type=cyclic factor=PARALLEL_FACTOR
...
TimeLoop:
    ...
    ParallelLoop:
        for (int p = 0; p < PARALLEL_FACTOR; p++) {
            #pragma HLS UNROLL

            int i_part = t * PARALLEL_FACTOR + p;
